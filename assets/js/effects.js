export function initializeEffects() {
    ...
    document.querySelectorAll('.project-card').forEach(el => applyTilt(el, { intensity: 10, scale: 1.05 }));
    document.querySelectorAll('.skill-item').forEach(el => applyTilt(el, { intensity: 15, scale: 1.02 }));
    document.querySelectorAll('.timeline-item').forEach(el => applyTilt(el, { intensity: 12, translateZ: 40, perspective: 1500 }));
    
    const aboutBox = document.querySelector('#about .neon-border');
    if (aboutBox) applyTilt(aboutBox, { intensity: 20, useShine: true, breakpoint: 992 });
}

function applyTilt(el, { intensity = 20, scale = 1, translateZ = 0, perspective = 1000, useShine = false, breakpoint = 0 }) {
    let bounds = null;
    let rafId = null;
...
function initializeCustomCursor() {
    ...
    window.addEventListener('mousedown', () => pressed = true, { passive: true });
    ...
    function render(time) {
        ...
        if (visible) {
            ...
            for (let i = len - 1; i >= 0; i--) {
                ...
                let col = i % 2 === 0 ? `rgba(15, 240, 252, ${op})` : `rgba(0, 255, 65, ${op})`;
